//
// Created by BlackBox on 23/8/2023.
//
...
*******************************************************************/
bool SerialInterface::openSyn(string serial_name, unsigned char baud_rate, unsigned char parity,
                              unsigned char byte_size, unsigned char stop_bits) {
    if (!openSyn(std::move(serial_name))){
        return false;
    }
    DCB dcb;
...
DWORD SerialInterface::readData(char *buffer, int length) {
    ...
    ret = ReadFile(hCom, buffer, length,
                   &writeSize, NULL);
