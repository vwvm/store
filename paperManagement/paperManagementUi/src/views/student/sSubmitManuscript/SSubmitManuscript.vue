<template>
    <h4>过程文档</h4>
    <br/>
    <el-card v-for="(poAuditMark, index) in poAuditMarks">
        <h4> 过程文档{{ index + 1 }} </h4>
        <br/>
        <el-descriptions title="" :column="3" border v-if="poAuditMark.auditMark !== 2">
            ...
            <el-descriptions-item label="下载" label-align="right" align="center">
                <el-button type="primary" @click="downloadFile()"><a :href="minioBaseUrl + poAuditMark.url"
                                                                     download>下载</a>
                </el-button>
            </el-descriptions-item
            >
    ...
    <el-card>
        <h4> 提交过程文档{{ poAuditMarks.length + 1 }} </h4>
        <br/>
        <el-form
                 ref="ruleFormRef"
                 :model="otherParam"
        ...
        >
            <el-form-item label="题目" prop="data_name">
...
import { SuccessFilled, QuestionFilled, CircleCloseFilled} from "@element-plus/icons-vue"
...
const getAuditMark = async () => {
    const {code, data} = await downloadApi.getAuditMarkList(otherParam);
    console.log("data", data)
    if (code === 200) {
        checkAuditMark.value = false;
        poAuditMarks.value = data;
    } else {
